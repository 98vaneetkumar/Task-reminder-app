...
module.exports = {
  ...
  sendPushToIos: async function (notification_data) {
    console.log(
      "🚀 ~ file: helper.js:101 ~ notification_data:",
      notification_data
    );
    try {
      var options = {
        to: notification_data.receiverDeviceToken,
        token: {
      ...
      //       key Id: Q3NW9UXH2J
      // Team Id: 4XVQBWH9QF
      // Bundle Id: com.KeepCleanApp

      let notification = {
        message: notification_data.message,
        type: notification_data.type,
        // receiverName: notification_data.receiverName,
        senderName: notification_data.senderName,
        senderImg: notification_data.senderImg,
        taskId: notification_data.taskId,
      };
      var apnProvider = new apn.Provider(options);
      ...
      note.topic = "com.KeepCleanApp";
      note.taskId = notification_data.taskId;
      console.log(note, "---note--");

      const status = await apnProvider.send(
        note,
        notification_data.receiverDeviceToken
      );
      console.log("🚀 ~ file: helper.js:139 ~ status:", status);
      // .then((result) => {
      //   console.log("send result", result);
      // });
      return status;
    } catch (error) {
      console.log("🚀 ~ file: helper.js:138 ~ error:", error);
    }
  },
